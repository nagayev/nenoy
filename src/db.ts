...
async function getUserInfo(id: string): Promise<any[]> {
  ...
  result.notesCount = await client
    ...
    .find({ user_id: ObjectId(id), checked: true })
    .count();
...
async function getPosts(params: { type: number; page: number }) {
  ...
  await client
    .db(postsDB)
    .collection(postsCollection)
    .find({ checked: true, type: params.type })
    .skip(POSTS_PER_PAGE * (params.page - 1))
...
async function voteForComment(data): Promise<void | string> {
  await connectOnce();
